...
public class PulsarProduceAvro {
    @Data
    static class Person {
        String name;
    }
    ...
    public static void main(String[] args) {
        ...
        pipeline.apply(Create.of(messages).withCoder(new CustomCoder<>() {
            ...
            @Override
            public void encode(Person value, @UnknownKeyFor @NonNull @Initialized
            OutputStream outStream)
                    throws IOException {
                byte[] data = AvroSchema.of(Person.class).encode(value);
            ...
            public Person decode(@UnknownKeyFor @NonNull @Initialized InputStream inStream)
                ...
                return AvroSchema.of(Person.class).decode(data);
            }
        })).apply(write);
